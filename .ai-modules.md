...
### `src/ui/events_log.py`
**Role:** Events log UI panel and logging handler for real-time user feedback

**Key Classes:**
- `EventsLogHandler(logging.Handler)` - Custom logging handler that forwards INFO+ log records to the UI panel. Thread-safe via tkinter `after()`. Buffers records before panel is attached.
- `EventsLogPanel(ttk.LabelFrame)` - Scrollable text panel displaying event log messages with colour-coded log levels (WARNING in orange, ERROR/CRITICAL in red). Auto-scrolls to latest message, trims to MAX_LINES (500).

**Dependencies:**
- `tkinter`, `logging`

**When to Modify:**
- Changing events log display format or styling
- Adjusting max buffer or max lines
- Adding log level filtering in the UI

**Important Notes:**
- Handler is created in `main.py` as module-level `events_log_handler` and added to all configured loggers
- Panel is created in `App._create_widgets()` and attached via `events_log_handler.attach_panel()`
- Thread-safe: generator thread emits log records → handler schedules UI updates via `after(0, ...)`
...
```
main.py
    ├── EventsLogHandler (ui/events_log.py) [logging → UI bridge]
    └── App (core/app.py)
        ├── EventsLogPanel (ui/events_log.py)
...
**Need to change events log panel?** → `src/ui/events_log.py`
