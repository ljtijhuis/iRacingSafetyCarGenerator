...
- `logger.debug()`: Detailed diagnostic information (per-loop data, internal computations)
- `logger.info()`: User-visible events shown in the Events Log panel (state transitions, detections with car numbers, safety car deployment, wave arounds, commands sent). Keep INFO messages concise and user-friendly — use car numbers (`#42`) not driver indices.
...
**Events Log Panel:** INFO+ messages from all configured loggers are displayed in a scrollable UI panel (`src/ui/events_log.py`). The `EventsLogHandler` is added to each logger in `setup_logging()` and bridges Python logging to tkinter via `after()`. This is in addition to the existing file logging — both handlers receive the same messages.

**Guidelines for INFO log messages:**
- Use car numbers (`#42`, `#7`) instead of driver indices for user-facing messages
- Include which cars are involved and why (e.g., "Detected 2 stopped car(s) on track: #42, #7")
- Avoid per-loop noise — event registration, cleanup, and per-second checks should be DEBUG
- State transitions, threshold triggers, wave arounds, EOL commands, and safety car deployment should be INFO
